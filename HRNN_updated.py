...
neg = glob.glob(img_filepath + 'negative/*.mp4')               #### negative examples - ACCV
pos = glob.glob(img_filepath + 'positive/*.mp4')                 #### positive examples - ACCV
...
def make_dataset(rand):
    seq1 = numpy.zeros(2*(len(rand), 99, 144, 256))   ### create an empty array to take in the data
    for i,fi in enumerate(rand):                    ### for each file...
        ...
        #    t = pickle.load(open(fi, 'rb'))
        	...
        	t = hori_flipped_load_set(fi)         ### otherwise, if it's pickled data, load the pickle
        if t.shape==(99,144,256):                  ### double check to make sure the shape is correct, and accept
